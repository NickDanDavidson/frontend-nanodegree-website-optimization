...
var resizePizzas = function(size) { 
  ...
  function changeSliderLabel(size) {
    switch(size) {
      case "1":
        // OPTIMIZATION: I changed document.querySelector to document.getElementById
  ...
  // Taken from Cam's video in Lesson 5 of Browser Rendering Optimization
  function changePizzaSizes(size) {
    var newWidth;

    // OPTIMIZATION: I removed the determineDx function and replaced it with
    // this switch case, which uses a % based new width
    switch(size) {
      case "1":
        newWidth = 25;
        break;
    ...
    // OPTIMIZATION: I stored the pizza elements in a variable and used
    ...
    var listLength = pizzaList.length;
...
/* OPTIMIZATION: I moved the querying of elements outside the for loop and outside
 * the function so that the var items is called only once and not every time
 * the updatePositions function is called. I also moved the calculation of the left
 * property outside the loop, and since there are only 7 unique left properties, I
 * used a similar technique as the phases to hopefully cut down on scripting time, or
 * at least not have it calculated during the loop.
 */
var items = document.getElementsByClassName('mover');
var basicLefts = [];
for (var j = 0; j < 7; j++) {
  var basicLeft = 256 * j;
  basicLefts.push(basicLeft);
}
var firstTime = true;
...
function updatePositions() {
  ...
  /* OPTIMIZATION: I moved the calculation of scroll position and phases outside the
   * loop to cut down on scripting time. Since there are only 5 unique phases, I
   * put their values in an array and call them during the loop.
   */
  ...
  // These two variables keep track of where in the list I should access.
  var whichPhase = 0;
  var whichLeft = 0;

  for (var i = 0; i < items.length; i++) { 
    // I could not figure out how to use the translateX property instead, but
    // the office hours notes said it's only a very minimal improvement and not
    // required to pass specifications if I am meeting the mark of 60fps. I do,
    // however, welcome feedback on how to use translateX in this case.
    items[i].style.left = basicLefts[whichLeft] + 100 * phases[whichPhase] + 'px';
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  /* OPTIMIZATION: I moved the querying outside of the for loop because it only
   * needs to be called once. I also changed to document.getElementById from
   * document.querySelector
   */
  var movingPizzas = document.getElementById("movingPizzas1");
  // Trying to figure out the height of the user's window to render number of
  // pizzas appropriately
  var innerHeight = window.innerHeight;
  var numPizzas;
  console.log(innerHeight);
  if (innerHeight > 700) {
    numPizzas = 32; // 8 (number of cols) * 4 (number of visible rows)
  } else if (innerHeight > 500) {
    numPizzas = 24;
  } else if (innerHeight > 260) {
    numPizzas = 16;
  } else {
    numPizzas = 8;
  }
